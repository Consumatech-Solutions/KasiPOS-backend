...
export class ChangeStoresIdToUuid1770930000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE "stores" ADD COLUMN "id_new" uuid DEFAULT uuid_generate_v4()`);
    await queryRunner.query(`UPDATE "stores" SET "id_new" = uuid_generate_v4() WHERE "id_new" IS NULL`);
    ...
    const childTables = [
      ...
      { table: 'vouchers', fk: 'FK_vouchers_store_id', uniqueConstraint: 'UQ_vouchers_store_code' },
    ...
    for (const { table, fk, uniqueConstraint } of childTables) {
      ...
      await queryRunner.query(`ALTER TABLE "${table}" ADD COLUMN "store_id_new" uuid`);
      ...
      await queryRunner.query(`ALTER TABLE "${table}" DROP COLUMN "${storeIdCol}"`);
      await queryRunner.query(`ALTER TABLE "${table}" ADD COLUMN "${storeIdCol}" uuid`);
      await queryRunner.query(`UPDATE "${table}" SET "${storeIdCol}" = "store_id_new"`);
      await queryRunner.query(`ALTER TABLE "${table}" DROP COLUMN "store_id_new"`);
      const notNullTables = ['parcels', 'marketplace_orders', 'vouchers', 'transactions', 'stock_adjustments', 'purchase_orders'];
      if (notNullTables.includes(table)) {
        await queryRunner.query(`ALTER TABLE "${table}" ALTER COLUMN "${storeIdCol}" SET NOT NULL`);
      }
    ...
    await queryRunner.query(`ALTER TABLE "stores" DROP CONSTRAINT IF EXISTS "stores_pkey"`);
    await queryRunner.query(`ALTER TABLE "stores" DROP COLUMN "id"`);
    await queryRunner.query(`ALTER TABLE "stores" RENAME COLUMN "id_new" TO "id"`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    throw new Error('Down migration not implemented: reverting store id from uuid to int requires data mapping.');
  }
}
