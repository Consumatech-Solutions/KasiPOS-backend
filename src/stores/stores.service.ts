...
export class StoresService {
  ...
  async createRoleTransfer(
    ...
    if (store.ownerId !== currentUserId) {
      throw new ForbiddenException(
        'Only the store owner can request a role transfer',
      );
    }
    ...
    if (newOwner.storeId !== store.id) {
      throw new BadRequestException(
        'The selected user is not a staff member of this store',
      );
    }
    if (newOwner.role !== UserRole.STAFF) {
      throw new BadRequestException(
        'The selected user must be a staff member (role: staff)',
      );
    }
    ...
    await this.sendRoleTransferApprovedSms(
      result.newOwner,
  ...
  async adminChangeStoreAdmin(dto: ChangeStoreAdminDto): Promise<{
    ...
    if (hasUserId) {
      ...
      if (u.id === oldOwner.id) {
        throw new BadRequestException(
          'New store admin must be a different user',
        );
      }
      newOwner = u;
    } else {
      const existing = await this.usersService.findByPhone(
      ...
      const resetToken = await this.authService.createStoreAdminResetToken(
        newOwner.id,
      );
      const userAppUrl =
        this.configService
          .get<string>('FRONTEND_URL_SMS')
          ?.split(',')[0]
          ?.trim() || 'http://localhost:9002';
    ...
    if (result.previousOwner.phone) {
      await this.smsService
        .send(result.previousOwner.phone, oldMsg)
        .catch(() => {});
    }
  ...
  async adminCreate(adminCreateStoreDto: AdminCreateStoreDto): Promise<Store> {
    ...
    const savedStore = await this.storesRepository.save(
      this.storesRepository.create(payload),
    );
    if (adminCreateStoreDto.ownerId) {
  ...
  async adminUpdate(
    id: string,
    adminUpdateStoreDto: AdminUpdateStoreDto,
  ): Promise<Store> {
    const store = await this.findOne(id);
  ...
  async assignStore(dto: AssignStoreDto): Promise<{
    ...
    return {
      ...
      message:
        'Store assigned. Store admin will receive an SMS with temporary password and reset link.',
    };
  }
}
